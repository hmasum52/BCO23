...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for org1..."
    fi
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for org2..."
    fi
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    createOrg1 # call the function in registerEnroll.sh

    infoln "Creating Org2 Identities"

    createOrg2 # call the function in registerEnroll.sh

    infoln "Creating Orderer Org Identities"

    createOrderer # call the function in registerEnroll.sh
...
CLI_DELAY=3
# channel name defaults to ittchannel(itt -> invitation to tender)
CHANNEL_NAME="ittchannel"
